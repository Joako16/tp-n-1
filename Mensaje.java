...
public class Mensaje
{
    ...
    {
        if (otro == null)
            throw new IllegalArgumentException("El mensaje del parámetro no puede ser nulo.");
        int i = 0;
        boolean iguales = true;
        while((i < cantLineas())){
            if(otro.cantLineas() == cantLineas()){
                if(!(otro.obtenerLinea(i).equals(obtenerLinea(i)))){
                    return false;
                }    
            }
            else{
